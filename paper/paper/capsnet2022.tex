...
The Capsule Neural Network (CapsNet)~\cite{nips/SabourFH17} is often cited as technical model for computer vision tasks, but moreover, and as the authors state specifically, the CapsNet is the instantiation of a biologically inspired, hypothetical system, that aims to dynamically construct a part-whole hierarchy from an image scene. This hierarchy is also called parse-tree, as  a complex objects within the scene is deconstructed into smaller and simpler objects, that stand in part-whole relation to each other.

This distinction is crucial as this defines CapsNets conceptually and not formally or technically.

% BEGINNING OF MOTIVATION
% Why was it introduced? [= motivation 1.]
As the Convolutional Neural Network (ConvNet)~\cite{neco/LeCunBDHHHJ89}, the predominant model for computer vision at that time, was found to suffer from several limitations, including
...
and a general lack of interpretability in the networks decision making process, the CapsNet was specifically designed to overcome some of these limitations and demonstrated SOTA classification accuracy on MNIST with less parameters and stronger robustness to affine transformations, compared to their ConvNet`s baseline. This work sparked a flood of follow-up research.
...
From there we investigate the question of how the CapsNet can be scaled and how the implementation is sufficient to yield a parse-tree.
This is the question of sufficiency.

Out investigation reveals:
\begin{itemize}
	\item There is a mismatch between the concept of the CapsNet and its implementation.
	\item Scaling with this particular technical version is hard.
	\item Scaling the backbone does not yield any benefits
	\item Scaling the CapsNet has severe issues.
	\item No sign of a parse tree
\end{itemize}

We give an overview of related work and model advancement where appropriate, but note, that a exhaustive investigation, including all versions of the CapsNets, is difficult as the literature is vast and diverse. However our claims are general enough to cover many implementations out there.

We think there is a elephant in the room, that needs to be addressed!
