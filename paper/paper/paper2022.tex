...
\title{Demystifying Capsule Neural Networks or Capsule Neural Networks do not learn a parse-tree!}
...
\begin{abstract}
  We show, that CapsNet architectures that are based on softmax routing by agreement with and squashing activation are flawed
...
\begin{itemize}
	\item Intro to what CapsNets are. The idea and concept.
	\begin{itemize}
		\item From a conceptual perspective: The idea of Hinton.
	...
	\end{itemize}
	\item General positive attitude towards CNs also in popular science.
	\item Promised benefits by citing various references.
	\item Contrary to benefits, some limitations found in literature and observation that CpasNet simply do not scale well. [Related Work]
	\item Münchner paper to general insuperiority of CapsNet
	\item Paper that shows that routing does not yield a benefit when it comes to accuracy and can be beaten by uniform and random routing
	\item So far attention has not been paid to the parse-tree claim, that is what we do!
	\item Shine lite on the routing by agreement routing. wha it is and why is it necessary. [PARSE TREE]
	\item Observations so far: Common CN architectures use a CNN backbone, followed by a single layer of CapsNet.
	\item It was found, that routing does not yield a general benefit in this setting.
	\item Also a single layer of CapsNet does not yield a parse tree.
	\item A complex parse tree requires multiple layers of capsules.
	\item We focus on the routing by agreement with softmax and squashing function
	\item List some CapsNet papers, that do that
	\item Show common denominator of that papers
	\item Routing-By-Agreement: Strong couplings with strong agreement limits the depth of the network.
	\item Argue that only strong coupling coefficients yield a parse-tree.
