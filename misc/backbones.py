...
def get_fcn8i22o32(ch_last, **kwargs):
    # ~ 50k
    planes = [8, 16, 32, ch_last]
    blocks = [1, 2, 2, 1]
    model = FCN(planes=planes, blocks=blocks, **kwargs)
    model.fc = torch.nn.Identity()
    return model
...
def get_fcn8i11o32(ch_last, **kwargs):
    # ~ 50k
    planes = [8, 16, 32, ch_last]
    blocks = [1, 1, 1, 1]
    model = FCN(planes=planes, blocks=blocks, **kwargs)
...
def get_fcn8i1o16(ch_last, **kwargs):
    # ~ 50k
    planes = [8, 16, ch_last]
    blocks = [1, 1, 1]
    model = FCN(planes=planes, blocks=blocks, **kwargs)
...
def get_backbone(backbone: str, **kwargs):
    ...
    elif backbone == "FCN8i22o32":
        model = get_fcn8i22o32(**kwargs)
    elif backbone == "FCN8i11o32":
        model = get_fcn8i11o32(**kwargs)
    elif backbone == "FCN8i1o16":
        model = get_fcn8i1o16(**kwargs)
    else:
