...
from effcn.models import AffnistEffCapsNetWORec
...
def eval_model(model, device, data_loader, config, func_margin):
    model.eval()
    epoch_loss = 0
    epoch_correct = 0
    epoch_total = 0
    ...
    for epoch_idx in range(1, config.train.num_epochs + 1, 1):
        ...
        if (epoch_idx % config.freqs.valid == 0) or (config.train.num_epochs == epoch_idx):
            loss_mnist_valid, acc_mnist_valid = eval_model(
                model, device, dl_mnist_valid, config, func_margin_loss)
            ...
            loss_affnist_valid, acc_affnist_valid = eval_model(
                model, device, dl_affnist_valid, config, func_margin_loss)
