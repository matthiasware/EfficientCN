...
def main(BATCH_SIZE=16,LEARNING_RATE=5e-4,mean=[127.5, 127.5],std=[127.5, 127.5] ):
    ##################################
    #Get & Preprocess data
    ...
    for epoch_idx in range(1, NUM_EPOCHS +1):
        ...
        #track best acc value & epoch
        if best_acc < (epoch_correct/epoch_total):
    ...
    #Show the best epoch
    print("best_epoch: {:.0f}".format(best_epoch))
    stats["best_epoch"].append(best_epoch)
    print("best_acc: {:.5f}".format(best_acc))
    stats["best_acc"].append(best_acc.item())

    #Save results
    stats["best_epoch"].append(best_epoch)
    stats["best_acc"].append(best_acc.item())

    file = open(p_run / 'a_results', 'wb')
    pickle.dump(stats, file)
    file.close()

    f = open(p_run / "a_best_result.txt", "w")
    f.write("best_epoch: " + str(best_epoch))
    f.write("\n")
    f.write("best_acc: " + str(best_acc.item()))
    f.close()    


if __name__ == '__main__':
    #main(BATCH_SIZE=16,LEARNING_RATE=5e-4,mean=[191.7811,193.0594],std=[45.2232, 44.2558])
    main(BATCH_SIZE=128,LEARNING_RATE=5e-4,mean=[127.5, 127.5],std=[127.5, 127.5])
    #main(16,5e-3)
    #main(32)
    #main(64)
    #main(128)
